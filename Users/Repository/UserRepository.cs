...
namespace Users.Repository
{
    public class UserRepository
    {
        ...
        public LoginAttempts? GetLastLoginAttempt(string ip)
        {
            return _dataContext.LoginAttempts.Where(la => la.email == ip).OrderByDescending(la => la.CreatedAt).FirstOrDefault();
        }

        public int RemoveAllLoginAttempts(string ip)
        {
            _dataContext.LoginAttempts.Where(la => la.email == ip).ExecuteDelete();
